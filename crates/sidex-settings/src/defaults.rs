...
fn add_editor_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "editor.semanticHighlighting.enabled", json!("configuredByTheme"));
    ...
    ins(m, "editor.quickSuggestions", json!({"other": true, "comments": false, "strings": false}));
    ...
    ins(m, "editor.wordBasedSuggestionsMode", json!("matchingDocuments"));
    ...
    ins(m, "editor.colorDecoratorsActivatedOn", json!("clickAndHover"));
    ...
    ins(m, "editor.find.seedSearchStringFromSelection", json!("always"));
    ...
    ins(m, "editor.bracketPairColorization.independentColorPoolPerBracketType", json!(false));
    ...
    ins(m, "editor.unicodeHighlight.ambiguousCharacters", json!(true));
    ins(m, "editor.unicodeHighlight.invisibleCharacters", json!(true));
    ins(m, "editor.unicodeHighlight.nonBasicASCII", json!("inUntrustedWorkspace"));
    ins(m, "editor.unicodeHighlight.includeComments", json!("inUntrustedWorkspace"));
    ins(m, "editor.screenReaderAnnounceInlineSuggestion", json!(true));
    ins(m, "editor.gotoLocation.multiple", Value::Null);
    ins(m, "editor.gotoLocation.multipleDefinitions", json!("peek"));
    ins(m, "editor.gotoLocation.multipleTypeDefinitions", json!("peek"));
    ins(m, "editor.gotoLocation.multipleDeclarations", json!("peek"));
    ins(m, "editor.gotoLocation.multipleImplementations", json!("peek"));
    ins(m, "editor.gotoLocation.multipleReferences", json!("goto"));
    ins(m, "editor.wordSeparators", json!("`~!@#$%^&*()-=+[{]}\\|;:'\",.<>/?"));
...
fn add_workbench_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "workbench.editor.enablePreviewFromQuickOpen", json!(false));
    ins(m, "workbench.editor.enablePreviewFromCodeNavigation", json!(false));
    ...
    ins(m, "workbench.editor.openSideBySideDirection", json!("right"));
...
fn add_file_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "files.exclude", json!({"**/.git": true, "**/.svn": true, "**/.hg": true, "**/CVS": true, "**/.DS_Store": true, "**/Thumbs.db": true}));
    ins(m, "files.watcherExclude", json!({"**/.git/objects/**": true, "**/.git/subtree-cache/**": true, "**/node_modules/**": true, "**/.hg/store/**": true}));
...
fn add_terminal_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "terminal.integrated.rightClickBehavior", json!("selectWord"));
    ins(m, "terminal.integrated.copyOnSelection", json!(false));
    ins(m, "terminal.integrated.drawBoldTextInBrightColors", json!(true));
    ins(m, "terminal.integrated.fastScrollSensitivity", json!(5));
    ins(m, "terminal.integrated.mouseWheelScrollSensitivity", json!(1));
    ins(m, "terminal.integrated.macOptionIsMeta", json!(false));
    ins(m, "terminal.integrated.macOptionClickForcesSelection", json!(false));
    ...
    ins(m, "terminal.integrated.tabs.hideCondition", json!("singleTerminal"));
    ...
    ins(m, "terminal.integrated.persistentSessionReviveProcess", json!("onExitAndWindowClose"));
    ins(m, "terminal.integrated.enableMultiLinePasteWarning", json!("auto"));
    ins(m, "terminal.integrated.minimumContrastRatio", json!(4.5));
}

#[allow(clippy::too_many_lines)]
fn add_search_defaults(m: &mut Map<String, Value>) {
    ins(m, "search.exclude", json!({"**/node_modules": true, "**/bower_components": true, "**/*.code-search": true}));
...
fn add_window_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "window.title", json!("${dirty}${activeEditorShort}${separator}${rootName}${separator}${appName}"));
...
fn add_debug_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "debug.internalConsoleOptions", json!("openOnFirstSessionStart"));
...
fn add_explorer_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "explorer.sortOrderLexicographicOptions", json!("default"));
...
fn add_extension_defaults(m: &mut Map<String, Value>) {
    ...
    ins(m, "extensions.closeExtensionDetailsOnViewChange", json!(false));
}
...
mod tests {
    ...
    fn count_defaults() {
        ...
        assert!(count > 250, "Expected 250+ defaults, got {count}");
    }
}
